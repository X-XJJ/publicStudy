...
[最香远程开发解决方案！手把手教你配置VS Code远程开发工具，工作效率提升N倍](https://www.cnblogs.com/NanoDragon/p/12899430.html)
...
- 模型参数：算法从数据中学习到的参数（如神经网络的权重，eg：若要学习函数f=ax+b中的a和b具体是多少，最后学到的a和b即模型参数）
- 超参数：人为设定的配置和参数（如学习率、层数）
...
- 张量，tensor = 多维数组or多维数据结构？？？
  ...
  - 计算机科学和机器学习中的张量——表示多维数据、多线性关系。
    - 在计算机科学和机器学习领域，张量通常指“多维数组”。这种用法源自于线性代数中的张量概念，但在这里张量更多地被用作数据结构，用于存储和处理数据。例如，在深度学习中，输入数据、模型的权重和神经网络的激活都可以表示为张量。
    - 零阶张量：标量。 一阶张量：向量。 二阶张量：矩阵。 三阶及以上张量：多维数组。

[什么是张量](https://blog.csdn.net/u013172930/article/details/144716925)
...
- 池化 = pooling，直译是池子、汇集、集中资源、合并。本质是对特征图进行降维（下采样）和抽象（提取主导特征），保留关键信息的同时减少计算量。其作用如同从细节照片中提取轮廓素描——保留主要特征，忽略次要细节。
...
- 激活函数 = 参考生物学的说法，用于激活神经元的函数，通常是非线性函数，Activation Function
  ...
  - sigmoid()：输入是“单个实数”或“对向量中的每个元素单独应用”，将之映射到区间(0,1)之间，函数表达式 sigmoid(x) = 1/[1+e^(-x)]。常用于 二分类 问题，输出结果表示 x属于某类的概率值。
  - softmax()：输入是“向量”，对输入进行归一化，每个元素的输出值都在区间(0,1)之间，且所有输出元素的总和为 1。函数表达式 softmax(xi) = e^xi/(∑e^xj)。常用于 多分类 问题，输出结果表示 xi属于某类别的概率，即 将一个实数向量or矩阵 映射到一个概率分布上。
  - ReLU()：将输出的负值压缩为0，保留正值


- 消融实验 = 用于 评估模型中各个模块的重要性 的实验，ablation study 翻译直译来的。本质思路还是 控制变量，和其他实验的设计是一样的，有控制地移除或禁用模型的某些部分。
