...
- 激活函数 = 参考生物学的说法，用于激活神经元的函数，通常是非线性函数，Activation Function
  - 生物学中，神经元在接收到足够强的刺激时才会被激活，即产生动作电信号。类似地，在人工神经网络中，激活函数决定了每个神经元是否应该被激活，即是否应该向网络的下一层传递信息。
  - softmax()：归一化，即进行概率转换，将一个实数向量或矩阵映射到一个概率分布上。具体来说，Softmax 函数可以确保经过它处理的输出值在 0 到 1 之间，并且所有输出值的总和为 1，这使得这些输出可以被解释为概率。
  - sigmoid()：将输出限制在0~1之间
  - ReLU()：将输出的负值压缩为0，保留正值
