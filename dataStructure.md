...
- 遍历：X序+中序，确定一个二叉树
  - 深度遍历
    ...
    - 序列、二叉树互转
    - [无脑秒解！已知先/后序遍历与中序遍历，求后/先序遍历。](https://www.bilibili.com/video/BV1Xu411d7qf/)
    - [【纯干货】三分钟教会你遍历二叉树！学不会举报我！！](https://www.bilibili.com/video/BV1Ub4y147Zv)
...
- 平衡二叉树 AVL树
  ...
  - 大一统调整
    - 找到要做平衡调整的三个点：从 离插入点最近的不平衡结点 到 插入点 的【路径】上的前三个点，ps：不平衡结点的左右子树差＞1，即 插入点插入之后 变得最长的那一溜
...
- 最优二叉树 = 哈弗曼树 Huffman
  ...
  - 哈夫曼树生成：森林中，找两个根结点权值最小的树 作为新结点的左、右子树，新结点权值 = 左子树的根权值 + 右子树的根权值，以此类推，即 所求元素or字符都是 叶节点
  - 按此构造方法，哈夫曼树只有度为0和2的结点，即只存在 N0 和 N2，且 N0 = N2 + 1，即 字符节点 比 用来构造数的节点 多一个
  - 总节点数 = 奇数，为 n 个字符构造huffman，新建了 n-1 个结点（即N2），∴树共有 2n-1 个结点
  - k叉树按规则补空白节点
  - 哈弗曼编码
...
- 树与二叉树之间的转化：树节点的兄弟 转化为 该节点的右孩子、右孩子的右孩子……
- 树转二叉树：
  - （1）所有兄弟结点之间加一条连线
  - （2）对原来树中的线，每个节点只保留它与它第一个孩子的连线；
- 二叉树转树：反过来，先加线（恢复除了左边节点的节点们 的 兄弟身份），后删线（删除 这些兄弟在二叉树里的 右孩子身份）

- 森林：先序，中序
- 森林转二叉树
  - （1）森林中的树分别转成二叉树；
  - （2）将多个二叉树合并成一棵二叉树：第一棵二叉树不动，从第二棵二叉树开始，依次把后一颗二叉树的根结点作为前一颗二叉树的根结点的右孩子，用线链接起来。

[《数据结构C语言版》——树、森林与二叉树的转换(超详图解)<LeftMouse>](https://blog.csdn.net/weixin_44572229/article/details/125571516)
...
- 邻接表
  ...
  ~~~
  顶点H[0] →2|顶点1到顶点2的距离 →顶点1连的第二条边连向顶点4|顶点1到顶点4的距离...
    ### 2为顶点1的第一条边连向的顶点2在H[]中的下标
  顶点H[1] →顶点2的第一条边连向顶点4在H[]中的下标|顶点2到顶点4的距离 →...
  顶点H[2] →...
  ~~~
- 十字链表
  - 有向图，链式存储，顶点结点顺序存储
...
- 简单选择
  - 表L = {有序L[]，Li，无序剩余L}
  - 一趟找出一个无序子表L中最小的元素，将之【与Li交换】，作为第i个元素进入有序子表末尾
...
-**交换**-|------|--------------|------------
冒泡排序  |√    |√            |√          |一趟一个最终位置
快速排序  |不稳  |O(n * log₂n) |O(log₂n)栈 |一趟一个最终位置，最坏时间O(n²)，最坏空间O(n)→因为递归？
-**选择**-|------|--------------|------------
简单选择  |不稳  |√            |√          |一趟一个最终位置
