...
- 平衡二叉树
  ...
  - 旋转：插入or删除后调整最小不平衡子树
    ...
    - RR-左单：结点A的右孩子的右子树中插入，需A左旋转一次
    - LR-先左后右：结点A的左孩子的右子树中插入，先左转左子树，再右转A结点
    - RL-先右后左：结点A的右孩子的左子树中插入，先右转右子树，再左转A结点
  - 平均查找长度O(log₂n)
- 最优二叉树/哈弗曼树 Huffman
  - 生成：在森林中，找权值最小的两个根结点的树作为新结点的左、右子树，新结点权值 = 左子树的根权值 + 右子树的根权值，以此类推
...
- 最短路径
  ...
  - 任意两两顶点最短：弗洛伊德 Floyd 各顶点最短路径算法
    - 时O(n^3)，找各个顶点互相之间的最短路径和路径长度
    - 构造n*n矩阵A寻找，迭代插点，方阵更替，变化矩阵内部内容，A(-1) A(0) A(1) ... A(n-1)
      - 初始A(-1)即该图的邻接矩阵，每迭代一次，从Vi到Vj的最短路径中多考虑一个顶点Vk，看和原路径比谁更短
      - A[i][j] = 点i到点j的 边权值 or 路径权值和，A(-1)初始化，A(k)[i][j] = min{ A(k-1)[i][j], A(k-1)[i][j] + A(k-1)[k][j] }
- 拓扑排序
  - DAG 有向无环图 Directed-Acyclic-Graph
    - DAG的所有点输出的序列 = 活动序列
  - AOV网 Activity-On-Vertex-Network
    - AOV网 = 顶点代表活动的DAG图，vertex-顶点，活动只能按顺序来
  - 拓扑排序：满足条件DAG图的顶点组成的一个序列，不唯一
    - 排出的顺序和边指向顺序相同，即 Vi→Vj，则排序Vi必在Vj前
    - 排序：找无前驱的顶点为起点，输出之，再去掉起点和起点连的边们，找下一个顶点
    - 矩阵O(n^2+e)，表(n+e)，算法利用栈，入度=0则入栈
- 关键路径
  - AOE网 Activity-On-Edge-Network
    - AOE网 = 带权有向图，顶点表示事件，边表示活动，权值表示活动开销，edge-边
    - AOE只有一个源点（入度=0），一个汇点（出度=0），中间全是过程的顶点
  - 性质（理解关键路径的基础）
    - 只有事件发生后，事件后的活动才能开始；只有事件前的所有活动都完成后，事件才能开始
  - 关键路径：具有最大路径长度（权值）的路径，关键路径上的活动为关键活动，关键路径的开销 = 完成整个工程的最短时间
  - 找关键活动
    - 顶点 = 事件，边 = 活动，事件Vi to 事件Vj，中间经过 活动K
    - 事件最早开始 = max{直接前驱事件最早 + 活动持续}
    - 事件最迟开始 = min{直接后继事件最迟 - 活动持续}
...
- B树
  ...
  - 结点结构：{n P0 K1 P1 K2 P2 ...}，n为结点中关键字的个数，Ki为关键字-左小右大，Pi指向儿子
...
- 堆排序  ：将L[]看成一颗完全二叉树的顺序存储结构，排序过程=调整初始树为堆的过程
  ...
  - 用途：实现优先级队列
## 归并、基数
- 归并：将≥2个的有序表组合为一个新的有序表，将n个元素的待排序表看成n个有序子表
  - 二路归并，两两归并
...
直接插入 |√    |O(n^2)         |√
折半插入 |√    |O(n^2)         |√
希尔排序 |不稳  |O(n^(1.3))     |√          |最坏时间O(n^2)
--交换---|------|---------------|------------
冒泡排序 |√    |O(n^2)         |√          |一趟一个最终位置
快速排序 |不稳  |O(n * log(2)n) |O(log(2)n)栈|一趟一个最终位置|最坏时间O(n^2) 最坏空间O(n)
--选择---|------|---------------|------------
