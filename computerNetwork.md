...
- 概述五层
  - 物理层
    - 介质命名与识别，单位为bit，传输比特流，信号的编码、译码，标准EIA-232C等
  - 数据链路层
...
## 流量控制、可靠传输机制
- 可靠传输机制：确认 + 超时重传
- 滑动窗口机制：滑动窗口包括发送窗口、接收窗口，设采用n比特对帧编号，即帧序列号（帧编号）占多少位，如3位可产生8个序列号
...
- 网络地址转换 NAT
  - 用于对外隐藏内部管理的IP地址，只用于局域网LAN，在现有IP中划出一部分作为私有地址
    - A类私有：10.0.0.0 ~ 255.255.255.255，10.全0 ~ 全255
    - B类私有：172.16.0.0 ~ 172.31.255.255，172.16.全0 ~ 172.31.全255
    - C类私有：192.168.0.0 ~ 192.168.255.255，192.168.全0 ~ 192.168.全255
  - NAT转换表
