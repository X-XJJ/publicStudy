...
- 分组转发
  ...
  - 首部定长占20字节（20B=160b）
    - 版本号占4位，首部长度占4位，区分服务8位
    - 总长度占16位（IP数据报封装成帧必须≤以太网成帧的最大传送单元MTU=1500B）
    - 标识占16位：同一数据报分片之后的标识号相同，不同数据报的标识加一
    - 标志占3位，第二位为DF Don't-Fragment，最低位为MF More-Fragment
      - DF = 0 表示允许分片，MF = 1 表示其后还有分片，MF = 0 表示是该数据报的最后一个分片
    - 片偏移占13位：该片在原分组中的相对位置
    - 生存时间TTL占8位，表示数据报在网络中可通过的路由器数的最大值
    - 协议占8位，6表示TCP，17表示UDP
    - 首部检验和占16位，只校验分组首部，不验数据部分
    - 源地址占32位，目的地址占32位
...
- IPv4地址
  ...
  - 分五类的IP地址
    - A类：类别位 0   ，网络号8b 1~126              ，网络数 2^7 - 2 ，容纳主机数 2^24 - 2
...
- 网络地址转换 NAT
  ...
  - 只用于局域网LAN，NAT转换表-实现内部地址与全球IP地址对应 = {本地IP:端口} 映射 {全球IP:端口}
  - 已经到端口，NAT工作在传输层
...
- 应用进程在传输层的标识：软件端口
  ...
  - 服务器端使用端口号
    - 熟知端口号：0~1023，互联网地址指派机构IANA 指派给TCP/IP最重要的一些应用程序
      - 常见服务端口：FTP 21，SSH 22，TELNET 23，SMTP 25，DNS 53，TFTP 69，HTTP 80，SNMP 161
...
- 首部固定 20 字节，全双工通信，点对点，发送端and接收端设置缓存，累计确认
  - 2B源端口，2B目的端口
  - 4B序号seq：本报文段发送数据的第一个字节的序号（TCP数据流每个字节都编序号，起始序号连接时建立）
  - 4B确认号ack：希望收到对方的下一个报文段数据的第一个字节的序号
  - 4位首部长度/数据偏移：报文段数据距离报文段起始的距离
  - 6位保留字段
  - 1b紧急位 URG：=1时配合紧急指针，数据从第一个字节到紧急指针处为紧急数据，传送优先级高
  - 1b确认位 ACK：=1时确认号有效有意义，建立连接后所有传送的报文段ACK必为1
  - 1b推送位 PSH：=1时尽快向上交付应用进程
  - 1b复位位 RST：=1时TCP严重差错，必须释放连接再重新建立
  - 1b同步位 SYN：=1时表示连接请求（ACK=0）or连接接收报文（ACK=1）
  - 1b终止位 FIN：=1时要求释放连接
  - 2B窗口字段：当前允许对方发送的数据量，即给发送方设置的发送窗口，单位：字节
  - 2B校验和：校验首部、数据，加12B伪首部进行校验
  - 2B紧急指针：指出紧急数据字节数？
- TCP 连接管理
  - 连接建立、数据传送、连接释放，客户Client/服务器Server方式
  - TCP连接的端口：套接字socket/插口，唯一确定一条TCP连接
  - 相关：SYN 同步位，ACK 确认位，seq 序号（起始、随机），ack 确认号
  - 建立连接 三次握手
  ...
  - 释放连接 四次挥手
    ...
    - 一端A始发，FIN = 1，seq = u                          ，A端不能再发数据，B端还可以
    - 对端B收发，         seq = v，    ACK = 1，ack = u + 1，A到B连接释放，B还可以发数据
    - 对端B又发，FIN = 1，seq = w，    ACK = 1，ack = u + 1，B端不能再发数据
    - 一端A收发，         seq = u + 1，ACK = 1，ack = w + 1，A发确认后等待2MSL时间后，B到A连接释放
- TCP 可靠传输
  - 序号：保证数据有序
  - 确认
    - 利用确认号，TCP默认使用累计确认
    - 只确认到数据流中第一个丢失的字节、即期望收到的下一个报文段，而失序的报文先缓存起来
    - GBN（后退N帧ARQ协议）失序的就丢弃
  - 重传
    - 超时重传：RTTs = (1-a)*OLD + a*NEW
    - 冗余ACK
      - 收到＞期望序号的失序报文段时，发送一个冗余ACK指明下一个期待字节的序号
      - 收到对同一报文段的3个冗余ACK，认为该报文段之后直连的报文段已丢失，如收到3个1号的，则2号丟
      - 冗余ACK同时用在“拥塞控制”的“快重传”
- TCP 流量控制（利用滑动窗口机制）
  - 控制端到端用户之间，发送方发送速率、接收方读取速率的匹配，防止发送方使接收方缓存溢出
  - 接收窗口 rwnd：receive 即接收方报文头的“窗口”字段，发送方根据rwnd调整自己的发送窗口大小
  - 发送窗口实际上限 = Min｛rwnd，cwnd｝，单位：最大报文段长度MSS
  - 数据链路层的流量控制：定义相邻结点的流量控制，窗口大小不变
- TCP 拥塞控制
  - 全局性过程，防止网络中路由器、链路过载
  - 拥塞窗口 cwnd：congest 发送方估计当前网络的拥塞程度而确定，与网络带宽、时延有关
  - AIMD算法？additive increase multiplicative decrease 加法增大、乘法减小
  - 慢开始 + 拥塞避免（应对超时）
    - 门限阈值 ssthresh，n = 传输轮次 = n个往返时延RTT
    - 慢开始：“指数增长”，初始发送方拥塞窗口 = 1 →收到一个确认 cwnd增一倍，即 cwnd = 2^n
    - 拥塞避免
      - “加法增大”：当cwnd ≥ssthresh，收到一个确认 cwnd++
      - “乘法减小”：当出现超时，ssthresh = cwnd / 2
    - 网络拥塞处理：无论何时，检测到超时且 cwnd ≥2，就 ssthresh = cwnd / 2 后 cwnd = 1
  - 快重传 + 快恢复（应对冗余ACK，慢开始、拥塞避免的改进）
