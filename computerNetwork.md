...
- 报文分类 = 数据部分SDU报文 + 控制信息部分报文
  - 控制信息部分：PCI，PDU =｛物理层-比特，数据链路层-帧，网络层-分组，传输层-报文｝
  - 报文逐层封装，高层报文为低层的数据部分
...
- 自动重传请求
  - 停止等待协议(1,1)
  - GBN协议：滑动窗口 + 后退N帧，(1, (2^n)-1)
  - SR协议：滑动窗口 + 选择重传，(2^(n-1), 2^(n-1))
## 介质访问控制
- 信道划分 f，time，w，code，d，mu啥啥？？英文
  - FDM-频分多路复用，TDM-时分多路，WDM-波分多路，CDM-码分多路-正交向量内积=0
- 随机访问：争用信道，尽可能避免冲突
  - ALOHA协议：
  - 时隙ALOHA协议，每个时隙开始才能发送帧
  - CSMA协议：1-坚持CSMA，非坚持CSMA，p-坚持CSMA-信道空闲时每个时隙以p的概率发送数据
  - CSMA/CD协议
    - 总线以太网，载波侦听、碰撞检测
    - 先听后发、边听边发、冲突停发、随机重发
...
- IEEE 802.11对应无限局域网
  - 有固定基础设施：基站-接入点AP，基本服务集-BSS，移动站，AP接入主干分配系统DS-扩展服务集ESS
  - 无固定基础设施：AD-Hoc-平等
- 网络接口卡NIC：每块网卡对应唯一MAC地址，工作于物理层，控制主机对介质访问
## 广域网 WAN
- 涉及OSI下三层（物理 + 数据链路 + 网络），强调数据传输
- 交换机组成节点，只能连通同类型网络 ≠互联网
- 核心问题：路由选择，分组转发
- 协议
  - PPP：面向字节，由LCP NCP组成，将IP数据报分装到串行链路
    - 链路控制协议LCP，向下建立和管理链路
    - 网络控制协议NCP，向上为多种网络提供服务
  - HDLC：面向比特，配置方式-平衡/非平衡，站类型-主站/从站/复合站，响应方式-正常响应/异步平衡/异步响应
