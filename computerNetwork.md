...
- 网络地址转换 NAT
  - 用于对外隐藏内部管理的IP地址，只用于局域网LAN，在现有IP中划出一部分作为私有地址
    ...
    - C类私有：192.168.0.0 ~ 192.168.255.255，192.168.全0 ~ 192.168.全255，192.168.内
  - NAT转换表
    - 存放｛本地IP:端口｝对应｛全球IP:端口｝的映射关系
    - 将私有网段（专用网内部的可重用地址）转换为公用地址
    - 实现内部地址与全球IP地址对应
    - 过程：IP、端口能定位到数据报最终要发到哪里，IP决定了外网应该找到哪个路由，端口决定了转换到内网后应该找哪台主机？？？，，，，20年47题
  - NAT工作在传输层，已经到端口层级
- 子网划分
...
- TCP 拥塞控制
  ...
  - 慢开始 + 拥塞避免（应对超时）
    - 流程：慢开始 →过门限后加法增大 →拥塞后乘法减小 →慢开始
    - 门限阈值 ssthresh，n个传输轮次 = n个往返时延RTT
    - 慢开始
      - “指数增长”：当cwnd＜ssthresh，收到一个确认时 cwnd++
      - 若某时刻cwnd = x，则可按发送时延发送 x 个报文段，故在一个往返时延RTT内，若发送方连续发送，能收到 x 个返回确认，∴cwnd增加了x，即增加到2x
      - 指数增长：经过1个RTT后，cwnd = 2*cwnd，下一次依旧是cwnd = 2*cwnd（此时cwnd已经增长过一轮），此为随RTT的指数增长，第n论的cwnd = 初始cwnd*2^n，如 cwnd = 1，1个RTT后为2，两轮后为4，三轮后为8
    - 拥塞避免
      - “加法增大”：当cwnd ≥ssthresh，经过一个传输轮次RTT时 cwnd++
      - “乘法减小”：当出现超时（即拥塞），ssthresh = cwnd / 2
    - 网络拥塞处理：无论何时，检测到超时且 cwnd ≥2，就 ssthresh = cwnd / 2 后 cwnd = 1
  - 快重传 + 快恢复（应对冗余ACK，改进）
    - 流程：慢开始 →过门限后加法增大 →3个ACK后乘法减小 →快重传、快恢复
    - 快重传：不等超时，利用ACK冗余，收到3个针对同一报文冗余ACK时直接重传对方期望的报文段
    - 快恢复：“乘法减小”，连续收到3个ACK冗余时，ssthresh = cwnd / 2 且 cwnd = cwnd / 2
  - 当cwnd＜ssthresh时，使用慢开始算法
  - 当cwnd＞ssthresh时，改用拥塞避免算法（加法增大、乘法减小）
