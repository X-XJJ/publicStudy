...
- 主要性能指标（7个）
  - 速率：码元传输速率，数据传输速率，单位 b/s、kb/s、Mb/s、Gb/s，换算10^3
    - 传输速率 = 发送的快慢
    - 传播速率 = 电磁波信道中传播速度
  - 带宽：频带宽度Hz，最高数据率b/s = 比特率bps（每秒传送的比特数bit-per-second，ps即per second），信道所能达到的的传输速率极限
  - 吞吐量 = 在单位时间内实际通过某个网络（或信道、接口）的数据量
  - 总时延 = 发送or传输时延 + 传播时延 + 处理时延 + 排队时延
    - 一般计算时，不考虑处理时延、排队时延
    - 发送or传输时延 = 分组长度/信号宽度（帧长/数据传输速率bps？理解 柱高=体积/底面积）
    - 传播时延 = 信道长度/电磁波在信道上的传播速率（时间=距离/速度）
  - 发送周期：发送方从发送数据开始，到收到对方的第一个确认帧，总共耗用的时间，为一次发送周期
    - 发送周期 = 数据帧发送时延 + 数据帧传播时延 + 确认帧发送时延 + 确认帧传播时延
  - 信道利用率 = 数据帧发送时延/发送周期 = 有效数据发送时间 在发送周期中占的比率（有效数据不包括确认帧）
    - 信道利用率是对于发送方而言的，相同数据大小下，发送窗口越大，信道利用率越高
    - https://blog.csdn.net/suliangkuanjiayou/article/details/107390604?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-1.control&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-1.control
...
- 概述五层
  - 物理层
    - 介质命名与识别，传输比特流，单位为bit，信号的编码、译码，标准EIA-232C等
  - 数据链路层
    - 点到点通信，单位为帧，差错控制、流量控制、传输管理，MAC、LLC子层
    - 协议：停止-等待，PPP，STP，SDLC，HDLC等
    - LLC = 数据链路层上层-逻辑链路控制，Logical Link Control
    - MAC = 数据链路层下层-介质访问控制，Media Access Control
  - 网络层
    - 主机间通信，单位为数据报，差错控制、流量控制、拥塞控制
    - 协议：IPv4/6，ICMP，ARP，OSPF，RARP等
  - 传输层
    - 主机间两个进程通信，单位为TCP报文段、DP用户数据报，差错控制、流量控制、端到端可靠连接、数据传输管理、服务质量
    - 协议：TCP，UDP
  - 应用层
    - 协议：DNS，FTP，HTTP，POP3，SMTP，IMAP等
- 搞个表！
...
会话层     |    |合应用层？
表示层     |    |合应用层？
应用层     |    |DNS，FTP，HTTP，POP3，SMTP，IMAP，
...
- 交换方式：电路交换，报文交换，分组交换
  ...
  - 分组交换：存储转发，大数据块划分为固定小块，方式：数据报、虚电路
    - 数据报：无连接，不可靠，每个报文都需完整目的地址，每个分组独立进行路由选择、路由转发，无序到达、最大努力交付
    - 虚电路：有连接，可靠，第一个报文需要完整目的地址、后续只要短虚电路号，建立时进行路由选择、后续就由同一路由转发，保证有序，不需分配带宽
- 传输介质：非导向、导向
  - 导向传输介质：
    - 同轴电缆：10BASE，10Mb/s，总线型，百米级，曼彻斯特编码
    - 双绞线（T）：100BASE，100Mb/s，星型，百米级，曼彻斯特编码，表示为 100BaseT：数据传输率100bps，Base-基带传输，T-双绞线
    - 光纤（F）：1000BASE，1Gb/s，点对点，千米级，曼彻斯特编码
    - 无线：无线电波，微波、红外、激光
- 传输设备
...
# 三、数据链路层
- 功能：为网络层提供服务
  - 无确认无连接-以太网Ethernet，有确认无连接-无线通信，有确认面向连接-通信要求高
  - 链路管理：建立，维持，释放
  - 帧相关：帧定界，帧同步，透明传输
...
- 设备：网桥、交换机，可隔离冲突域
  - 冲突域：域内结点同时通信时，可能发生冲突，如一个CSMA/CD网络中两台计算机同时通信，信号碰撞？？
  - 网桥：多个以太网连接为一个网段，透明网桥-非最佳路由，源路由网桥-最佳路由
  - 交换机：多端口网桥，本网段内-直通式，不同网段-存储转发
- 最大传输单元 MTU：Maximum Transmission Unit，指一种通信协议的某一层上面所能通过的最大数据包大小(以字节为单位)
## 差错控制
...
## 流量控制、可靠传输机制
- 可靠传输机制：确认 + 超时重传
- 滑动窗口机制：滑动窗口包括发送窗口、接收窗口，窗口是指帧的序号范围。设采用n比特对帧编号，即帧序列号（帧编号）占多少位，如3位可产生8个序列号
  - 设 采用 n 比特 对帧编号，则窗口大小 ≤2^(n-1)
  - 发送窗口：在任意时刻，发送方都维持了一个连续的允许发送的帧的序号范围，窗口大小 ≤2^(n-1)
  - 接收窗口：在任意时刻，接收方也维持了一个连续的允许接收的帧的序号范围
- 由滑动窗口看协议
  - 停等协议：发送窗口 = 1，接收窗口 = 1，即(1,1)
  - 后退n帧协议：发送窗口＞1，接收窗口 = 1，即(1, (2^n)-1)？？
  - 选择重传协议：发送窗口＞1，接收窗口＞1，发送 + 接收 ≤2^n，∴接收≤2^(n-1)，即(2^(n-1), 2^(n-1))
- 自动重传请求 ARQ，是一种错误纠正协议 Automatic-Repeat-reQues，包括停止等待ARQ协议和连续ARQ协议，机制包括错误侦测(Error Detection)、正面确认(Positive Acknowledgment)、逾时重传(Retransmission after Timeout)与负面确认继以重传(Negative Acknowledgment and Retransmission)等
  - 停止-等待协议(1,1) stop-and-wait
    - 每发送完一个分组就停止发送，等待对方的确认，收到确认后再发送下一个分组
    - 简单，信道利用率低
  - GBN 后退N帧ARQ协议 Go-Back-N，(1, (2^n)-1) 滑动窗口协议
    - 发送方可以不等待确认帧、连续发送发送窗口范围内的多个帧，流水线中未确认的分组数不能超过最大允许数N帧
    - 接收对方确认帧只允许按顺序收，设发出某帧后又发了N帧，若超时没收到该帧的确认帧，则认为该帧出错or丢失，需要重新发送该帧、以及在发送该帧后发过的N帧，即“后退N帧”
  - SR 选择重传协议 (2^(n-1), 2^(n-1))
    - 接收方发现某帧出错后，其后继续送来的正确的帧虽然不能立即递交给接收方的高层，但接收方可收下来，存放在一个缓冲区中，同时要求发送方重新传送出错的那一帧
    - 一旦收到重新传来的帧后，就可以原已存于缓冲区中的其余帧一并按正确的顺序递交高层
    - 减少浪费，接收方需要大缓冲区

  - 以上分类好像不太对，以后整理
  - ref：https://www.cnblogs.com/yifeichongtian/p/10091235.html
## 介质访问控制
...
- 随机访问协议：争用信道，尽可能避免冲突
  ...
  - ：SMA/CD ~/冲突检测 Collision-Detection
    - 用于有线网络，总线以太网，载波侦听、碰撞检测
    - 先听后发、边听边发、冲突停发、随机重发
    - 总线传播时延影响大，2T内无法确定争用期，最小帧长原理
    - 二进制指数退避算法，重传次数↑，则推迟的时间范围↑
...
- 特殊的LAN、拓扑
  - 以太网
    ...
    - 广播发送信息，MTU限制1500字节，最小帧长64B
      - 标准以太网数据帧为1518B，头部信息占14B，尾部校验和FCS占4B，剩1500B留给上层协议传输数据
...
- 广播域：域内结点可以接收到同样的广播消息
- 插播：中继系统层次（设备们）
  - 物理层：    中继器，集线器Hub；只连接、啥也不隔离
  - 数据链路层：网桥、交换机；隔离冲突域（碰撞域）
  - 网络层：    路由器；隔离冲突域、广播域
  - 网络层以上：网关
    - 网关：到底是啥定义？？网络与网络之间连接的“关口”，就像房间与房间之间的门，是不同网络之间分隔连接的进出口
    - 目前很多局域网采用路由器接入网络，此时网关的具体含义就是路由器的ip
...
- 分组转发
  - IP数据报分片
  - 根据不同下层网络对数据长度的限制，每个路由都有可能将一个数据报 分成 不同长度的 多个分片
    - 分片の首部 + 数据の总大小 ≤数据链路层的最大传送单元MTU，如 以太网MTU = 1500字节，广域网MTU≤567字节
    - 分片大小：要求为 8B 的整数倍（∵首部中 片偏移の单位为 8B）
  - 首部定长：20字节（20B=160b）
    - 4b版本号
    - 4b首部长度，计算单位为4B
    - 8b区分服务
    ...
    - 32b源地址：32位，目的地址占32位
  - 首部计算单位汇总
    - IP数据报首部关于长度的标记数，有其各自的计算单位
    - 4位首部长度：单位 4B，最大首部长度 = (2^4 -1)×4B = 60B
    - 16位总长度：单位 1B
    - 13位片偏移：单位 8B，如片偏移为3，则片偏移实长24B
  - 转发：直连-直接交付，非直连-间接交付
- IPv4地址
...
- 网络地址转换 NAT
  - 用于对外隐藏内部管理的IP地址，只用于局域网LAN，在现有IP中划出一部分作为私有地址
    - A类私有：10.0.0.0 ~ 255.255.255.255，10.全0 ~ 255.全255，10.~255.
    - B类私有：172.16.0.0 ~ 172.31.255.255，172.16.全0 ~ 172.31.全255，172.16.~172.31.
    - C类私有：192.168.0.0 ~ 192.168.255.255，192.168.全0 ~ 192.168.全255，192.168.
  - NAT转换表
    - 存放｛本地IP:端口｝对应｛全球IP:端口｝的映射关系，实现内部地址与全球IP地址对应
    - 将私有网段（专用网内部的可重用地址）转换为公用地址
    - 过程：目的IP、端口能定位到数据报最终要发到哪里，IP决定了外网应该找到哪个路由，端口决定了转换到内网后应该找哪台主机？？？，，，，20年47题
    - 内网A数据报发到网关，NAT将 源ip、端口转换为 对外的ip、端口，NAT表加一条。报文目的ip、端口不变，请求外网服务。外网服务返回数据报，NAT将 返回给A的数据报的 目的ip、端口 转换为A在内网的ip、端口
    - 实现：NAT路由器必须：
    传出数据报：将每个传出数据报的（源IP地址，端口号）替换为（NAT IP地址，新端口＃）
     远程客户端/服务器将使用（NAT IP地址，新端口＃）作为目标地址进行响应
     记住（在NAT转换表中）每个（源IP地址，端口号）到（NAT IP地址，新端口＃）转换对
     传入数据报：在每个传入数据报的dest字段中替换（NAT IP地址，新端口＃），并在NAT表中存储相应源IP地址，端口号）
  - NAT工作在传输层，已经到端口层级
- 子网划分
...
- 传输层功能
  - 复用和分用
    - 复用：不同进程可以用同一个传输层协议
    - 分用：接收方的传输层能把去首部的数据正确交付到应用进程
  - 对报文包括首部、数据部分进行差错检测（网络层只查IP首部）
  - 同时提供2种传输协议（网络层同时只能存在一种）
    - 面向连接 TCP 可靠传输：适于可靠性更重要时，文件传输协议FTP、电子邮件SMTP、超文本传输协议HTTP、远程登录telnet等
    - 无连接 UDP 不可靠传输：适于实时性更重要时，小文件传送协议TFTP、DNS、SNMP、实时协议RTP等
- 应用进程在传输层的标识：软件端口
  - 传输层通过端口判断将数据送到何处，长度 16 bit ，能表示2^16个端口号
  - 服务器端使用端口号：0~49151
    - 熟知端口号：0~1023，互联网地址指派机构IANA 指派给TCP/IP最重要的一些应用程序
    - 登记端口号：1024~49151，在IANA登记后可使用
    - 熟知中常见的服务端口：FTP-21，SSH-22，TELNET-23，SMTP-25，DNS-53，TFTP-69，HTTP-80，HTTPS-443，SNMP-161
      - 服务端口，客户请求服务报文的目的端口是80，服务器返回报文的源端口也为80，服务器在80端口进行http的收发
...
- 无连接，不可靠，尽最大努力交付，无拥塞控制
  ...
  - 报文：UDP数据报处理的最小单位
- 首部固定 8 字节：2B源端口，2B目的端口，2B长度（首部+数据），2B校验和（可选全0）
- UDP校验
  - 数据报前增加 12B 伪首部 = 源IP + 目的IP + 0 + 17 + UDP长度
    - 伪首部：用来生成校验和，是抽象出来的一种数据结构，并不真实存在
  - 首部、数据部分，一起校验
  - 查错不纠错，有错丢弃
...
- 首部固定 20 字节，全双工通信，点对点，发送端and接收端设置缓存，累计确认
  ...
  - 6位 保留字段
  - 1b 紧急位 URG：URG=1时 配合紧急指针，数据从第一个字节到紧急指针处为紧急数据，传送优先级高
  - 1b 确认位 ACK：ACK=1时 确认号有效有意义，建立连接后所有传送的报文段ACK必为1
  - 1b 推送位 PSH：PSH=1时 尽快向上交付应用进程
  - 1b 复位位 RST：RST=1时 TCP严重差错，必须释放连接再重新建立
  - 1b 同步位 SYN：SYN=1时 表示连接请求（ACK=0）or连接接收报文（ACK=1）
  - 1b 终止位 FIN：FIN=1时 要求释放连接
...
- TCP 连接管理
  ...
  - 相关：SYN 同步位，ACK 确认位，FIN 终止位，seq 序号（起始、随机），ack 确认号-用于确认收到的对方序号
  - 建立连接：三次握手（考虑改表格）
    - 客户端：SYN = 1，seq = x，                          发送连接请求报文段，进入SYN_SEND状态
    - 服务器：SYN = 1，seq = y，    ACK = 1，ack = x + 1，收信息，回一个确认报文段
    - 客户端：         seq = x + 1，ACK = 1，ack = y + 1，收信息，回一个确认报文段，进入established状态
  - 释放连接：四次挥手
    ...
    - MSL：？？？
  - 延伸：三次握手耗时2个RTT，？？第三次握手时，客户端已经可以传数据了，所以第二个RTT的后半部分，服务端就可以传回客户端所需的内容
  - 主机对目标ip进行http请求，到收到index页面，此段时间为2个RTT，20年40题，正确解析在评论里
...
- TCP 可靠传输
  ...
  - 重传机制
    ...
    - 冗余ACK
      - 收到大于期望序号的失序报文段时，发送一个冗余ACK指明下一个期待字节的序号
      - 收到对同一报文段的3个冗余ACK，认为该报文段之后直连的报文段也已丢失，如收到3个1号的，则算2号丟
      - 冗余ACK同时用在“拥塞控制”里的“快重传”
- TCP 流量控制（利用滑动窗口机制）
  - 控制端到端用户之间，发送方发送速率、接收方读取速率的匹配，防止发送方使接收方缓存溢出
  - 接收窗口 rwnd：r即receive 即接收方报文头的“窗口”字段，发送方根据rwnd调整自己的发送窗口大小
  - 发送窗口 swnd：swnd实际上限 = Min｛rwnd，cwnd｝，单位：最大报文段长度MSS
  - 数据链路层的流量控制：定义相邻结点的流量控制，窗口大小不变
- TCP 拥塞控制
  - 全局性过程，防止网络中路由器、链路过载
  - 发送方的拥塞窗口 cwnd：c即congest 取决于发送方估计的当前网络的拥塞程度，动态变化，与网络带宽、时延有关
  - AIMD算法？additive increase multiplicative decrease 加法增大、乘法减小
  - 慢开始 + 拥塞避免（应对超时）
    - 流程：慢开始 →拥塞避免（过门限后加法增大，超时后乘法减小 →慢开始）
    - 门限阈值 ssthresh 状态变量，n个传输轮次 = n个往返时延RTT
    - 慢开始
      - 当cwnd＜ssthresh，收到一个ACK确认时 cwnd++
        - 若某时刻cwnd = x，则可按发送时延发送 x 个报文段，故在一个往返时延RTT内，若发送方连续发送，能收到 x 个返回确认，由于收到一个ACK确认则cwnd++，∴cwnd增加了x，即增加到2x。（cwnd的值是几，就能发送几个数据报文段）
      - 指数增长：经过1个RTT后，cwnd = 2*cwnd，下一次依旧是cwnd = 2*cwnd（此时cwnd已经增长过一轮），此为随RTT的指数增长，第n轮的cwnd = 初始cwnd*2^n，如 设当前cwnd = 1，1个RTT后为2，两轮后为4，三轮后为8
      - 设cwnd=1，而发送窗口swnd等于拥塞窗口cwnd，因此发送方当前只能发送一个数据报文段（拥塞窗口cwnd的值是几，就能发送几个数据报文段），接收方收到该数据报文段后，给发送方回复一个确认报文段，发送方收到该确认报文后，将拥塞窗口的值变为2，发送方此时可以连续发送两个数据报文段，接收方收到该数据报文段后，给发送方一次发回2个确认报文段，发送方收到这两个确认报文后，将拥塞窗口的值加2变为4...（以此类推）
    - 拥塞避免
      - “加法增大”：当cwnd ≥ssthresh，每经过一个传输轮次RTT时 cwnd++（慢开始是收到ACK时cwnd++）
      - “乘法减小”：拥塞处理，当出现超时且cwnd ≥2，ssthresh = cwnd / 2 后 cwnd = 1，进行慢开始
        - 一段时间后，丢失的x个报文段的重传计时器超时了，发送发判断可能出现拥塞，更改cwnd和ssthresh.并重新开始慢开始算法
  - 快重传 + 快恢复（应对丢包，改进）
    - 流程：慢开始 →3个ACK后快重传、快恢复 →回到拥塞避免
    - 快重传
      - 要求接收方在收到一个失序报文段后，立即发出重复确认（即冗余ACK）
      - 收到3个针对同一报文冗余ACK时，不等重传计时器到时，直接重传对方期望的报文段
    - 快恢复：“乘法减小”，连续收到3个ACK冗余时，ssthresh = ssthresh / 2 后 cwnd = ssthresh，然后执行拥塞避免算法
  - 慢开始门限ssthresh状态变量用法
    - 当cwnd＜ssthresh时，使用慢开始算法
    - 当cwnd＞ssthresh时，改用拥塞避免算法
    - 当cwnd = ssthresh时，慢开始与拥塞避免算法任意
  - 实际应用中这几种算法同时存在，分情况使用。慢开始之后，看遇到的情况调用拥塞避免、快重传、快恢复。其中 拥塞避免 内，又存在两种情况（加法增大、乘法减小）
  - 分类也有问题，之后整理
  - https://blog.csdn.net/xiaolonggezte/article/details/80425481
