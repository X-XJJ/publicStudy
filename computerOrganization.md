...
- 存储器 M：主存（内存） + 辅存（外存）
  ...
  - 存储字 = 存储单元内的一串二进制代码
  - 存储字长 = 存储单元的位数 = 一个字节（1Byte=8bit）or字节的偶数倍
    - 字和字节
      - 1字节（Byte）= 8位（bit）
...
- 运算速度
  ...
  - CPU时钟周期：是时钟脉冲信号的宽度，以相邻状态单元间の组合逻辑电路の最大延迟为基准决定
    - = 1秒 ÷时钟频率，是CPU的最小时间单位
...
- Cache地址映射：将主存的一块 映射（复制）到 Cache的哪一块上 的问题
  ...
  - 理解？？？
    ...
    - CPU发出请求，给出一个主存地址A，若Cache命中，则人为对A进行划分，按划分内容找到对应Cache块进行操作；若Cache不命中，则使用A访问主存

    - 理解？？？即 就主存地址一个，按需看待？其实是先有主存地址A，为解决速度不匹配而引入Cache，∵二进制的进位特性，使得可以人为划分一下A，规定各分区表示的含义，再将该含义下的数作为对应的Cache的地址，最终使得可以通过A的不同分区可以找到对应Cache块，也是 从而“创造”出对应的Cache地址？
  ...
  - 组相联映射
    ...
    - 将主存地址视为Cache地址时的划分结构：{ 主存字块标记 | 组地址 | 字块内地址 }
