...
- Cache读写策略：使Cache内容和主存内容保持一致，主要情景是 改变了Cache块的内容后 如何改对应的主存块的内容
  - 全写法（命中） + 非写分配法（不命中）
    ...
    - 全写法：CPU对Cache写命中，将数据同时写入Cache和主存，要替换块时新块直接覆盖原块
    - 非写分配法：CPU对Cache写不命中时，只直接写入主存，不调块
  - 写回法（命中） + 写分配法（不命中）
    - 改Cache优先，均需增加“脏位”表示数据变化
    - 写回法：CPU对Cache写命中，只修改Cache内容，当块被换出时才写回主存
    - 写分配法：CPU对Cache写不命中时，先调块到Cache，再更新Cache块（后续就连接上写回法了）
- Cache失效：义务失效，容量失效，冲突失效
- Cache中除了数据项，每块数据另有标记项，如 有效位、标记位、脏位？？有效位-表示该块是否已用，标记位-标记存的是主存中哪个块即主存块号？，脏位-表示该块数据是否被写改变内容，。
