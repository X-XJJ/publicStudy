...
定点数:
    在定点数中,小数点的位置不用特定的表示出来,它是是有一个约定的固定位置
    如果无需关心其正负性,那么符号位将用于表示数值,无符号数所能表示的数的数量与有符号数所能表示的是一样的
    
    我们以32位的一个数值为例
    其中,我们规定定点数的内存结构为,第1位为符号位,第2-9位为整数位,10-32位为浮点位
    假设我们有一个小数8.25
    那么0(符号位)1000(整数位)01(2^-2)=8.25
...
浮点数:
    ...
    浮点数的存储原理:
        ...
        我们先看传统的十进制科学计数法的表现:
            ...
            10进制数*乘以10,小数点向右移一位.
            同理,二进制移动一位则是乘除以2
        上述讲述了如何将二进制科学计数法化.而其存储结构也就可以一一对应了.
