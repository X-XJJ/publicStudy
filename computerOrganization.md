...
- 存储器的性能指标：存储容量，存储速度，单位成本
  ...
  - 单位成本：每位价格 = 总成本/总容量
  - 主存带宽 = 数据传输率 = 每秒从主存进出信息的最大数量，字/秒or字节/秒or位/秒
...
### Cache-主存层
- CPU、Cache、主存，之间两两双向直接访问
- Cache：解决CPU访问主存速度低的问题，保存主存中最活跃的若干个块的副本
- Cache即数据高速缓冲技术，基于局部性原理-时间局部性/空间局部性
- Cache命中率 = Cache命中次数 ÷(Cache命中次数 + 访问主存次数)，访问主存次数即Cache未命中
- 每个Cache块（即Cache行）都有标记项（有效位+标记位）表明该块来自主存中的哪个块
...
- Cache地址映射
  ...
  - 组相联映射：j = i mod Q
    ...
    - Cache分为大小相同的组，一个主存数据块可以装到固定组内的任意位置
    - 地址结构：{ 主存字块标记 | 组地址 | 字块内地址 }
- 多层Cache：第一层-数据Cache/指令Cache，第二层，第三层
- Cache替换算法：随机替换，先进先出FIFO，最近最少使用LRU（计数器命中清0否则加1），最不经常使用NRU（计数器命中加1）
- Cache读写策略：使Cache内容和主存内容保持一致
  - 全写法 + 非写分配法
