...
- 运算速度
  ...
  - CPI：执行一条？指令所需的时钟周期数，Clock-cycle-Per-Instruction，执行指令集所需的平均时钟周期数
...
- Cache地址映射：将主存的一块 映射（复制）到 Cache的哪一块上 的问题
  ...
  - 理解？？？
    - 如 先将存储器按字节编址（如 数据区为32B，即数据空间容量有32B，则每个字节有一个地址，共32=2^5个地址，地址为从0~31编号，写成二进制则是5位地址 00000~11111，∴需要5位二进制来存这32个地址，但由于按字节编址，∴至少需要1字节，1字节中的高3位可能就冗余了）
  ...
  - 组相联映射
    ...
    - 将主存地址视为Cache地址时的划分结构：{ 主存字块标记位 | 组地址（组号位） | 字块内地址（块内地址） }
