...
- OS状态：用户态（目态），核心态（管态）。属于 微内核 架构。（与“单体内核”区分，单体内核啥都在一起管理，无状态切换）
- 核心态：状态切换 + 内核
  - 状态切换/系统调用
...
- 调度算法性能比较准则：最大化CPU利用率、吞吐量，最小化周转、等待、响应时间
  - CPU利用率：尽可能高
...
- 目的：控制多道OS进程的异步性、临界资源的互斥访问
  ...
  - 临界区原则：空闲让进，忙则等待，有限等待，让权等待
    ...
    - 让权等待：当进程无法获得临界资源而不能进入临界区时，应立即释放处理机
...
- 信号量机制-解决互斥和同步问题
  ...
  - 进程同步实现：在两个进程中分别P和V，如需要进程P1中的语句X执行后、进程P2才能执行，则设置信号量S=0，在P1中语句X后进行V(S) 表示P1已完成一次X、把S增加1，进程P2中先P(S)申请一个资源来查询X是否完成、把S减少1试试看，若在P(S)时 S减少1后＜0 则P2阻塞等待，并循环检查S的当前值直到S可以足够减
...
- PV操作问题们
  ...
  - 读写者问题
    ...
    - 读者不互斥-添加循环
  - 哲学家就餐问题
    - 圆桌上一圈设5个哲学家，两两之间有一根筷子，一根一根地拿，拿到两根筷子可以进餐
    - 筷子资源的互斥信号量数组 chopstick[5] = {1,1,1,1,1}
    - 互斥取筷子动作的信号量 mutex = 1
...
- 自旋锁
## 程序出错
- 死锁：多个进程需要互相竞争不可剥夺资源，从而可能造成互相等待/阻塞的僵局，若无外力作用则无法向前推进
  - 充要条件：互斥，不可剥夺，请求保持，循环等待
- 死锁处理策略：预防，避免，检测和解除
- 死锁预防：死锁发生前，破坏死锁产生的条件之一or之几
  - 破坏互斥：不太可行
  - 破坏不可剥夺：常用于状态易保存和恢复的资源，如CPU寄存器、内存，一般不能用于打印机之类的
...
- 死锁避免：死锁发生前，在资源的动态分配过程中，防止系统进入不安全状态
  ...
  - 银行家算法：
    ...
    - 算法描述
      ...
      - 4、在此情况下执行系统的安全性检测算法，检查系统是否为安全状态
    - 安全性检测算法（安全状态）
...
- 动态/可变分区分配
  ...
  - 分配算法（首次-地址↑、最佳-容量↑、最坏-容量↓）
    ...
    - 首次适应 First-Fit：空闲分区按 地址递增 链接，顺序查找，选择满足要求的第一个空闲分区
    - 最佳适应 Best-Fit：空闲分区按 容量递增 链接，顺序查找，选择满足要求的第一个空闲分区
    - 最坏适应 Worst-Fit = 最大适应 Largest-Fit：空闲分区按 容量递减 链接，顺序查找，选择满足要求的第一个空闲分区
...
- 分页 存储管理
  ...
  - 逻辑地址划分结构 = { 页号P | 页内偏移量W }
    - 如 将存储器按字节编址，主存地址为32位，主存容量即物理内存4GB=2^32B，页面大小为4KB=2^2×2^12B
      - 页内偏移量（即页内地址）占12位，剩32-12=20位，即为页号占位
      - 页内偏移占12位 = 可表示页内地址2^12个 = 每页的页面大小为2^12为4KB
  ...
  - 页表项的大小问题
    ...
    - 当然，也可以选择更大的页表项 大小以至于让一个页面能够正好容下整数个页表项以方便存储（例如取成 4B，那么一页正好可以 装下 IK 个页表项），或者增加以前其他信息。
      ...
      - 注：Cache块大小 和 主存页大小 不一定相同？？19年46题
      - 从物理块中 读入指令，默认一条指令是一次性读入的，不论占位是否跨页，指令只产生一次缺页中断。读数据or进行操作则正常按一页一页看

  - 快表TLB/联想寄存器
    - 为页表而设的Cache，在高速缓冲存储器中、存放高频使用的若干页表项，基于局部性原理
      - 页表存储在内存中，取指or取数据CPU至少要访存两次（找页表项一次、找物理块一次），时间是平时的两倍
      - 快表放在Cache里，Cache本身集成在cpu里，找页表项时就不用访存了，至少只需访存一次-找物理块
  ...
  - 二级页表（多级页表）
    ...
    - 程序的虚拟地址结构划分 = { 页目录号 | 二级页号 | 页内偏移 }
    - 页目录表项 = { 页目录号（一级页号） | 对应存放二级页表的页框号 }
      - 其中，规定一个二级页表占用空间为一个物理块
    - 二级页表项 = { 二级页号 | 对应页存放的页框号 }
      - 二级页表项的物理地址 = 该页表所在页框号 + 页号 × 页表项长度 ？？
      - 20年46题（1）的第4小问，页表项的物理地址可以不是32位？？or只看有用的部分即前20位，后12位忽略了？？
...
- 分段存储管理
  - 为程序分配离散的地址空间，分配的基本单位：段，段从0开始编号，每个段有段号、段名
  - 按程序的逻辑分段，段内地址连续，段间地址不要求连续，有外碎片
  - 地址空间：二维的
  - 分段基本概念
    - 逻辑地址 = { 段号S | 段内偏移量W }，一定要显式给出才能求得物理地址
      - 段号位数n：每个进程最多可以分为2^n个段
      - 段内地址位数n：每个段的最大长度不超过2^n
    - 段表：一张段表记录一个程序分成的所有段在内存中的起始地址、段长度
    - 段表长度：一张段表内记录的段数，即 对应的那个程序划分成的段数
    - 段表项 = { 段号S | 段长C | 该段在主存的起始地址b }
      - 同页表类似，段号也具有“隐含”性质
    - 段表项长度 = 一个段表项占用的存储空间大小
    - 段表寄存器 = { 段表始址F | 段表长M }，S≥M则越界中断
    - 段号S所在的段表项地址 = 段表始址F + 段号S * 段表项长度
    - 段的物理地址 = 段所在物理块的起始地址b + 段内偏移W
- 段页式存储
  - 段表页表结合，一个进程建立一张段表，一个分段有一张页表
    - 一个进程只有一个段表，可有多个页表
  - 地址空间：二维的
  - 程序逻辑地址的结构划分 = { 段号S | 页号P | 页内偏移量W }
  - 段表项 = { 段号 | 页表长度 | 页表始址 | 等等...}
  - 页表项 = { 页号 | 页框号 | 等等...}
  - 段表寄存器 = { 段表始址F | 段表长M }
- 段表、页表寄存器作用：寻址，判断越界
...
- 页面置换算法：选择调出的页面
  ...
  - 时钟 CLOCK = 最近未使用 NRU Not-Recently-Used
    - 只考虑使用，每页设置一个使用位u，首次调入内存时 u = 1，页被访问时 u = 1
    - 调页步骤
      - 从当前位置开始，指针循环扫描缓冲区
    ...
    - 性能接近LRU
  - 改进的CLOCK
    - 考虑使用、页面修改，每页设置一个使用位 u 、修改位 m ，替换时首选最近没用且没变化的页
    - 调页步骤
...
- 文件分配
  ...
  - 索引分配：随机访问，文件长度可变，分为：直接索引，单级索引，多级索引，多级混合索引-UNIX中三级索引结构+分析文件大小计算
    ...
    - 簇：微软操作系统中，磁盘文件存储管理的最小基本单位，仅用于数据区
      - 文件存储中，等价说法：物理块？盘块？
      - 1簇 = 一组扇区，一个文件放在一或多个簇中
    - 这些号默认从0开始编号，0~9是10个，如簇号、柱面号、磁头号（盘面号）、扇区号（块号）
...
- 文件存储空间管理
  ...
  - 管理方式：空闲表法，空闲链表法，位示图法，成组链接法
    - 空闲表法
    - 空闲链表法
    - 位示图法
    - 成组链接法
- 物理存储
  - 磁盘结构概念：盘片，盘面（盘片可用上下两面，也可只用一面就是浪费），磁头（戳着盘面找数据），柱面，磁道，扇区（盘块）……
    - 扇区是磁盘可寻址的最小单位
    - 柱面：所有盘片上位置（半径）相同的磁道所组成的圆柱
  - 磁盘容量 = 磁盘柱面数×每个柱面の磁道数×每条磁道の扇区数×扇区大小
  - 磁盘地址表示：柱面号·磁头号（盘面号）·扇区号（块号）
  - 磁盘读写结构：磁头-磁道
  - 磁盘读写操作：寻道时间，延迟时间，传输时间
  - 磁盘，一旦开始运行，就不会停止等待，一直匀速转动着
- 磁盘调度算法
  - 先来先服务 FCFS，磁头可能随时变方向，如序列 50 55 40 56 45 60
  - 最短寻找时间优先 SSTF：会产生“饥饿”，磁头可能随时变方向，如序列 50 55 40 71 5
  - 扫描（电梯）SCAN：找当前移动方向上，离磁头最近的，没有才换方向
  - 循环（单向）扫描 C-SCAN：电梯 + 规定磁头只能单向移动 + 需要从起始端移动到末尾端
  - LOOK：改进SCAN，磁头到最远请求即可返回，不用到磁盘端点
  - C-LOOK：改进C-SCAN，磁头到最远请求即可返回，不用到磁盘端点
  - 旋转调度：考虑处理时延，同磁头不同扇区，不同磁头相同编号扇区，不同磁头不同编号扇区
- 个人OS的文件系统
...
- Spooling技术：空间换时间
  ...
  - eg：打印机设备的 输入输出井，谁点打印都会同意，进入缓存等着打，不会出现“打印机正在使用”而拒绝的情况
- IO性能相关
